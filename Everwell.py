...
q1='''
select * from 1097_db_iemr.t_everwellapi as api 
...
q9='''
select distinct b.ReceivedAgentID, b.CreatedBy from
...
where b.CreatedDate between date_format(subdate( now(),interval 1 week),'%%Y-%%m-%%d 00:00:00') and date_format(subdate( now(),interval 1 day),'%%Y-%%m-%%d 23:59:59');
'''
...
q8='''
select Language, count(distinct userid) as 'Agents_Count' from 1097_db_iemr.t_everwellapi
...
q2='''
select api.EverWellID,max(fb.LastModDate) as LastModDate,fb.SubCategory,fb.Comments from 1097_db_iemr.t_everwellapi as api 
inner join 1097_db_iemr.t_everwellfeedback as fb on fb.EAPIID=api.EAPIID
where (api.CreatedDate between date_format(subdate( now(),interval 1 week),'%%Y-%%m-%%d 00:00:00') and date_format(subdate( now(),interval 1 day),'%%Y-%%m-%%d 23:59:59'))
group by EverWellID;
'''
...
q4='''
select Language, count(*) as cnt from 1097_db_iemr.t_everwellapi
...
q5='''
select avg(b.CZcallDuration) as avg_call_duration from
(select Bencallid from
1097_db_iemr.t_everwellapi
group by Bencallid) as a
inner join 1097_db_iemr.t_bencall as b on a.Bencallid = b.BenCallID
...
q6='''
select count(distinct userid) from 1097_db_iemr.t_everwellapi
where createddate between date_format(subdate( now(),interval 1 week),'%%Y-%%m-01 00:00:00') and date_format(subdate( now(),interval 1 day),'%%Y-%%m-%%d 23:59:59')
and retryNeeded is true and isAllocated is true and userid is not null;
'''
...
q7='''
select SubCategory as 'Call_Outcome', count(SubCategory) as Count
...
q3= '''
call 1097_db_iemr.Pr_EverwellDailyReport(date_format(subdate( now(),interval 1 week),'%%Y-%%m-%%d 00:00:00'),date_format(subdate( now(),interval 1 day),'%%Y-%%m-%%d 23:59:59'))
'''
